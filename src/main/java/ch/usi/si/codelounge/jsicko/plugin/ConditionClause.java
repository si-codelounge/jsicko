...
class ConditionClause {
    ...
    private static Pattern clauseFormatRegexp = Pattern.compile("(\\!?)([a-zA-Z_0-9]+)");
    ...
    private ConditionClause(JavacUtils javac,  String clauseRep, ContractConditionEnum conditionType) {
        ...
        if (!clauseRepFormatMatcher.matches())
            throw new IllegalArgumentException("Clause specification name \"" + clauseRep + "\" is malformed - please match regexp " + clauseFormatRegexp.toString());
        this.javac = javac;
